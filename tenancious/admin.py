...
from .maker import create_model
...
class TenantAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        NewSite = create_model(f"{obj.name}_Site", parent=Site)
        NewStaff = create_model(f"{obj.name}_Staff", parent=Staff, target=NewSite)
        NewDesignation = create_model(f"{obj.name}_Designation", parent=Designation, target=NewSite)
        ...
        apps = admin.site.get_app_list(request, app_label="tenancious")
